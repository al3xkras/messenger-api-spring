...
public interface ChatRepository extends JpaRepository<Chat,Long> {
    ...
    @Transactional
    @Query("select chat from ChatUser chatUser join fetch chatUser.chat chat where chatUser.userId=?1")
    ...
    @Transactional
    @Query("select chat from ChatUser chatUser join fetch chatUser.chat chat where chatUser.userId=" +
